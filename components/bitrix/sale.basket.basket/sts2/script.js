 ...
 */
/*window.addEventListener('scroll', fixOrder, false);
window.addEventListener('onresizestart', fixOrder, false);
function fixOrder(){
    var buyBox=document.getElementById("jsjs");
    var a =buyBox.getBoundingClientRect();
    var fixCtn=document.getElementById("jsjs1");
    var b =fixCtn.getBoundingClientRect();
    var rightS= b.left;
    console.log(rightS);
	...
	if(a.top < 1){
        ...
        buyBox.style.top="0";
        buyBox.style.right=rightS+"px";
    }
    if(b.top > 1){
        buyBox.style.position="relative";
        buyBox.style.top="";
        buyBox.style.right="";
    }
}*/
...
function modalAlert(msg,st1button,nd2button) {
...
function checkOut()
{
	...
	$('.cartProduct__content .productInline__content').each(function(){
		...
		var count=$(this).find('.counter__ctn input').val();
		var name=$(this).find('.productInline__section--name').text();
...
function recalcBasketAjax(params)
{
	...
	{
		for (i = 0; items.children.length > i; i++)
			postData['QUANTITY_' + items.children[i].id] = BX('QUANTITY_' + items.children[i].id).value;
	}
	...
	BX.ajax({
		url: '/bitrix/components/bitrix/sale.basket.basket/ajax.php',
        //url: '/bitrix/components/orangerocket/ajax.php',
		method: 'POST',
		data: postData,
		dataType: 'json',
		onsuccess: function(result)
