...
<div class="section group group--el">
	<script type="text/javascript">
			...
			<div class="bx_block w100 vertical">
				<div class="bx_element">
					...
					<label for="PAY_CURRENT_ACCOUNT" id="PAY_CURRENT_ACCOUNT_LABEL" onclick="changePaySystem('account');" class="<?if($arResult["USER_VALS"]["PAY_CURRENT_ACCOUNT"]=="Y") echo "selected"?>">
						<input type="checkbox" name="PAY_CURRENT_ACCOUNT" id="PAY_CURRENT_ACCOUNT" value="Y"<?if($arResult["USER_VALS"]["PAY_CURRENT_ACCOUNT"]=="Y") echo " checked=\"checked\"";?> style='display:none'>
		...
		{
			//if($arPaySystem["ID"] == 23 && $isTestPay || $arPaySystem["ID"] != 23 && !$isTestPay)
			if (strlen(trim(str_replace("<br />", "", $arPaySystem["DESCRIPTION"]))) > 0 || intval($arPaySystem["PRICE"]) > 0)
			{
				?>
                    <div class="col col-6-tl w100">
                        <div class="radio<?if ($arPaySystem["CHECKED"]=="Y") echo " radio--selected";?>" data-qcontent="element__buttons__radio">
                            ...
                            <label class="radio__label" for="ID_PAY_SYSTEM_ID_<?=$arPaySystem["ID"]?>" onclick="BX('ID_PAY_SYSTEM_ID_<?=$arPaySystem["ID"]?>').checked=true;changePaySystem();">
                                <div class="bx_description">
