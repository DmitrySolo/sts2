...
if($arResult['PARTNER_XML_ID']){
    if(CModule::IncludeModule("highloadblock")){
        ...
        $result = $hlDataClass::getList(array(
            'filter' => array('UF_XML_ID' => $arResult['PARTNER_XML_ID']),
        ...
        if($res = $result->fetch()){
            $arResult['PARTNER_ID'] = $arResult['PARTNER_XML_ID'];
            $arResult['PARTNER_ID_1'] = $res['UF_ALT_XML_ID'];
            $arResult['PARTNER_ID_2'] = $res['UF_ALT_XML_ID_1'];
            $arResult['PARTNER_ID_3'] = $res['UF_ALT_XML_ID_2'];

            foreach($arResult["ORDER_PROP"]["USER_PROPS_Y"] as &$val){
                if(empty($val['VALUE'])||$val['IS_ZIP']=='Y')
                    switch ($val['CODE']){
                        case 'COMPANY':
                            $val['VALUE'] = $res['UF_COMPANY_NAME'];
                            break;
                        case 'COMPANY_ADR':
                            if(!empty($res['UF_JUR_ADDRESS'])) $val['VALUE'] = $res['UF_JUR_ADDRESS'];
                            break;
                        case 'INN':
                            $val['VALUE'] = $res['UF_INN'];
                            break;
                        case 'ZIP':
                            if(!empty($res['UF_INDEX'])) $val['VALUE'] = $res['UF_INDEX'];
                            break;
                        case 'PHONE':
                            if(!empty($res['UF_PHONE'])) $val['VALUE'] = $res['UF_PHONE'];
                            break;
                        case 'CITY':
                            $val['VALUE'] = $res['UF_CITY'];
                            break;
                        case 'ADDRESS':
                            if(!empty($res['UF_JUR_ADDRESS'])) $val['VALUE'] = $res['UF_JUR_ADDRESS'];
                            break;
