...
ISDEKservice::setTarifPriority(
    array( 137, 139, 16, 18, 11, 1, 3, 61, 60, 59, 58, 57, 83),
    array( 136, 62,138, 15, 17, 62, 63, 5, 10, 12)
);
...
{
	// auth
	protected static $account = 'b62fd71f538cd2de349dc5b033ec1bae'; //укажите логин
	protected static $key     = 'f3e259e1d137f7f2c8fe6bcfc4109095'; //укажите ключ
	

	protected static $tarifPriority = true;
	...
	//MODIFICIED
    public static function getPVZ_sts()
    {
        $arPVZ = self::getPVZFile();
        if ($arPVZ) {
            self::toAnswer(array('pvz' => $arPVZ));
        }
       return self::printAnswer_sts();
    }
    //ЕND
    ...
    public static function calc_sts($data)
    {
        ...
        $data['shipment']['cityToId'];


        if ($data['shipment']['cityToId']) {
            $answer = self::calculate_sts($data['shipment']);
    ...
    protected static function calculate_sts($shipment)
    {
        $headers = self::getHeaders();
	...
	//MOD
    protected static function printAnswer_sts()
    {
      return self::$answer  ;
    }
}
